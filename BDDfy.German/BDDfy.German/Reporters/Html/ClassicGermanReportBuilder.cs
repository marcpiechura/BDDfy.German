...
{
    public class ClassicGermanReportBuilder : IReportBuilder
    {
        ...
        public ClassicGermanReportBuilder()
        {
        ...
        private void HtmlHead()
        {
            using (OpenTag(HtmlTag.head))
            {
                ...
                AddLine(string.Format("<title>BDDfy Testergebnisse {0}</title>", _model.RunDate.ToShortDateString()));
            }
        }
        ...
        private void ResultSummary()
        {
            ...
            {
                AddLine("<h3 class='label'>Zusammenfassung:</h3>");

                using (OpenTag("<ul class='resultSummary'>", HtmlTag.ul))
                {
                    AddSummaryLine("storySummary", "Stories", _model.Summary.Stories);
                    AddSummaryLine("scenarioSummary", "Szenarios", _model.Summary.Scenarios);
                    AddSummaryLine("Passed", "Erfolgreich", _model.Summary.Passed);
                    AddSummaryLine("Inconclusive", "Unvollständig", _model.Summary.Inconclusive);
                    AddSummaryLine("NotImplemented", "Nicht implementiert", _model.Summary.NotImplemented);
                    AddSummaryLine("Failed", "Fehlgeschlagen", _model.Summary.Failed);
                }
            }
        }
        ...
        private void ExpandCollapse()
        {
            ...
            {
                AddLine("<a href='#' class='expandAll'>[alle aufklappen]</a>");
                AddLine("<a href='#' class='collapseAll'>[alle zuklappen]</a>");
            }
        }
        ...
        private void ResultDetails()
        {
            using (OpenTag("<div id='testResult'>", HtmlTag.div))
            {
                ...
                AddLine(string.Format("<p><span>Getestet am: {0}</span></p>", _model.RunDate));
            }
        }
        ...
        private void AddExamples(Scenario[] scenarioGroup)
        {
            ...
            {
                AddLine("<span class='example-header'>Beispiele:</span>");
                using (OpenTag(string.Format("<table class='examples' style='border-collapse: collapse;margin-left:10px''>"), HtmlTag.table))
                {
                    using (OpenTag("<tr>", HtmlTag.tr))
                    {
                        ...
                        if (scenarioResult == Result.Failed)
                            AddLine(string.Format("<th>Fehler</th>"));
                    }
